...
fights the vertical-pipeline model — not "needs one more verb," but the
verb-per-line, each-line-transforms-the-last shape itself doesn't fit how
...
what every current verb does. It's an open question whether that's just a
new *verb* (`EXPLORE` still fits the pipeline shape fine as a single step —
