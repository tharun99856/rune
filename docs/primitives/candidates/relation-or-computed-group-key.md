...
equal target - this value?), which no current verb expresses.
...
2. Extending `GROUP` to accept a *computed* key (e.g. `GROUP nums BY (target - value)`) rather than only a literal field — which might get most of the way there without any new verb at all.
