...
def format_complexity_report(top_k_kwargs=None, graph_kwargs=None):
    ...
    lines.append(f"  Naive      O(n log n) time, O(n) space:  {top_k['naive_time_ms']:9.2f} ms, {top_k['naive_peak_kb']:12,.1f} KB peak")
    lines.append(f"  Optimized  O(n log k) time, O(k) space:  {top_k['optimized_time_ms']:9.2f} ms, {top_k['optimized_peak_kb']:12,.1f} KB peak")
    lines.append(f"  Time ratio:   {top_k['naive_time_ms'] / top_k['optimized_time_ms']:.2f}x")
    lines.append(f"  Space ratio:  {top_k['naive_peak_kb'] / top_k['optimized_peak_kb']:.2f}x")
